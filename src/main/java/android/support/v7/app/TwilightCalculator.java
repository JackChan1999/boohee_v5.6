...
class TwilightCalculator {
    ...
    public void calculateTwilight(long time, double latiude, double longitude) {
        ...
        double solarLng = (1.796593063d + (((((double) meanAnomaly) + (0.03341960161924362d * Math.sin((double) meanAnomaly))) + (3.4906598739326E-4d * Math.sin((double) (2.0f * meanAnomaly)))) + (5.236000106378924E-6d * Math.sin((double) (IPhotoView.DEFAULT_MAX_SCALE * meanAnomaly))))) + 3.141592653589793d;
        double arcLongitude = (-longitude) / 360.0d;
        double solarTransitJ2000 = ((((double) (J0 + ((float) Math.round(((double) (daysSince2000 - J0)) - arcLongitude)))) + arcLongitude) + (0.0053d * Math.sin((double) meanAnomaly))) + (-0.0069d * Math.sin(PathListView.ZOOM_X2 * solarLng));
        ...
        if (cosHourAngle >= PathListView.NO_ZOOM) {
            this.state = 1;
            this.sunset = -1;
            this.sunrise = -1;
