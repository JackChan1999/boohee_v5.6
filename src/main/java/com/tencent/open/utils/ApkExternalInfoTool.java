...
public final class ApkExternalInfoTool {
    ...
    private static final ZipLong  a         = new ZipLong((long) ZipConstants.ENDSIG);
    ...
    private static byte[] a(RandomAccessFile randomAccessFile) throws IOException {
        ...
        while (read != (byte) -1) {
            if (read == bytes[0] && randomAccessFile.read() == bytes[1] && randomAccessFile.read
                    () == bytes[2] && randomAccessFile.read() == bytes[3]) {
                break;
            }
            length--;
            randomAccessFile.seek(length);
