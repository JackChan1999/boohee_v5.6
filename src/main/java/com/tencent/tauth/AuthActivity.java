...
public class AuthActivity extends Activity {
    ...
    private void handleActionUri(Uri uri) {
        ...
        if (string == null) {
        ...
        } else if (string.equals("shareToQQ") || string.equals("shareToQzone") || string.equals
                ("sendToMyComputer") || string.equals("shareToTroopBar")) {
            if (string.equals("shareToQzone") && SystemUtils.getAppVersionName(this, "com.tencent" +
                    ".mobileqq") != null && SystemUtils.compareQQVersion(this, SystemUtils
                    .QQ_VERSION_NAME_5_2_0) < 0) {
                mShareQzoneBackTime++;
            ...
            r0 = "";
            try {
                r0 = Util.parseJson(decodeUrl.getString("response")).getString
                        (SHARE_PRIZE_ACTIVITY_ID);
            } catch (Exception e) {
    ...
    private void execAuthCallback(Bundle bundle, String str) {
        ...
        if (auth != null) {
            if (str.indexOf("://cancel") != -1) {
                ...
                JSONObject decodeUrlToJson = Util.decodeUrlToJson(new JSONObject(), Util
                        .encodeUrl(bundle));
