...
public abstract class BaseApi {
    ...
    protected static final String  ACTIVITY_ENCRY_TOKEN = "com.tencent.open.agent" +
            ".EncryTokenActivity";
            ...
            this.mHandler = new Handler(Global.getContext().getMainLooper(), BaseApi.this) {
                public void handleMessage(Message message) {
                    if (message.what == 0) {
                        ...
                        TempRequestListener.this.mListener.onError(new UiError(message.what,
    ...
    protected Bundle composeCGIParams() {
        ...
        SharedPreferences sharedPreferences = Global.getContext().getSharedPreferences("pfStore",
                0);
        if (isOEM) {
            bundle.putString(Constants.PARAM_PLATFORM_ID, "desktop_m_qq-" + installChannel +
                    SocializeConstants.OP_DIVIDER_MINUS + "android" + SocializeConstants
                    .OP_DIVIDER_MINUS + registerChannel + SocializeConstants.OP_DIVIDER_MINUS +
                    businessId);
        } else {
            bundle.putString(Constants.PARAM_PLATFORM_ID, sharedPreferences.getString(Constants
                    .PARAM_PLATFORM_ID, "openmobile_android"));
        }
        return bundle;
    }
    ...
    protected void handleDownloadLastestQQ(Activity activity, Bundle bundle, IUiListener
            iUiListener) {
        StringBuilder stringBuilder = new StringBuilder();
