...
final class LoadAndDisplayImageTask implements Runnable, CopyListener {
    private static final String ERROR_NO_IMAGE_STREAM                         = "No stream for " +
            "image [%s]";
    private static final String ERROR_POST_PROCESSOR_NULL                     = "Post-processor " +
            "returned null [%s]";
    private static final String ERROR_PRE_PROCESSOR_NULL                      = "Pre-processor " +
            "returned null [%s]";
    private static final String ERROR_PROCESSOR_FOR_DISK_CACHE_NULL           = "Bitmap processor" +
            " for disk cache returned null [%s]";
    private static final String LOG_CACHE_IMAGE_IN_MEMORY                     = "Cache image in " +
            "memory [%s]";
    private static final String LOG_CACHE_IMAGE_ON_DISK                       = "Cache image on " +
            "disk [%s]";
    private static final String LOG_DELAY_BEFORE_LOADING                      = "Delay %d ms " +
            "before loading...  [%s]";
    private static final String LOG_GET_IMAGE_FROM_MEMORY_CACHE_AFTER_WAITING = "...Get cached " +
            "bitmap from memory after waiting. [%s]";
    private static final String LOG_LOAD_IMAGE_FROM_DISK_CACHE                = "Load image from " +
            "disk cache [%s]";
    private static final String LOG_LOAD_IMAGE_FROM_NETWORK                   = "Load image from " +
            "network [%s]";
    private static final String LOG_POSTPROCESS_IMAGE                         = "PostProcess " +
            "image before displaying [%s]";
    private static final String LOG_PREPROCESS_IMAGE                          = "PreProcess image" +
            " before caching in memory [%s]";
    private static final String LOG_PROCESS_IMAGE_BEFORE_CACHE_ON_DISK        = "Process image " +
            "before cache on disk [%s]";
    private static final String LOG_RESIZE_CACHED_IMAGE_FILE                  = "Resize image in " +
            "disk cache [%s]";
    private static final String LOG_RESUME_AFTER_PAUSE                        = ".. Resume " +
            "loading [%s]";
    private static final String LOG_START_DISPLAY_IMAGE_TASK                  = "Start display " +
            "image task [%s]";
    private static final String LOG_TASK_CANCELLED_IMAGEAWARE_COLLECTED       = "ImageAware was " +
            "collected by GC. Task is cancelled. [%s]";
    private static final String LOG_TASK_CANCELLED_IMAGEAWARE_REUSED          = "ImageAware is " +
            "reused for another image. Task is cancelled. [%s]";
    private static final String LOG_TASK_INTERRUPTED                          = "Task was " +
            "interrupted [%s]";
    private static final String LOG_WAITING_FOR_IMAGE_LOADED                  = "Image already is" +
            " loading. Waiting... [%s]";
    private static final String LOG_WAITING_FOR_RESUME                        = "ImageLoader is " +
            "paused. Waiting...  [%s]";
    ...
    public void run() {
        if (!waitIfPaused() && !delayIfNeed()) {
            ...
            try {
                ...
                runTask(new DisplayBitmapTask(bmp, this.imageLoadingInfo, this.engine, this
                        .loadedFrom), this.syncLoading, this.handler, this.engine);
    ...
    private boolean delayIfNeed() {
        ...
        L.d(LOG_DELAY_BEFORE_LOADING, Integer.valueOf(this.options.getDelayBeforeLoading()), this
                .memoryCacheKey);
        try {
    ...
    private boolean fireProgressEvent(int current, int total) {
        ...
        if (this.progressListener != null) {
            runTask(new 1 (this, current, total),false, this.handler, this.engine);
        }
        return true;
    }
    ...
    private void fireCancelEvent() {
        if (!this.syncLoading && !isTaskInterrupted()) {
            runTask(new 3 (this), false, this.handler, this.engine);
        }
    }
