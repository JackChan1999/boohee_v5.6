    ...
    public static CoderResult malformedN(ByteBuffer src, int nb) {
        ...
        switch (nb) {
            ...
            case 3:
                b1 = src.get();
                b2 = src.get();
                if ((b1 == -32 && (b2 & Opcodes.SHL_INT_LIT8) == 128) || isNotContinuation(b2)) {
                    i = 1;
                }
                return CoderResult.malformedForLength(i);
            case 4:
                b1 = src.get() & 255;
                b2 = src.get() & 255;
                if (b1 > 244 || ((b1 == SocializeConstants.MASK_USER_CENTER_HIDE_AREA && (b2 < 144 || b2 > Opcodes.REM_LONG_2ADDR)) || ((b1 == 244 && (b2 & SocializeConstants.MASK_USER_CENTER_HIDE_AREA) != 128) || isNotContinuation(b2)))) {
                    return CoderResult.malformedForLength(1);
                }
                if (isNotContinuation(src.get())) {
