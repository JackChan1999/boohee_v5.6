...
public class CirclePageIndicator extends View implements PageIndicator {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.mViewPager != null) {
            ...
            float threeRadius = this.mRadius * IPhotoView.DEFAULT_MAX_SCALE;
            float shortOffset = ((float) shortPaddingBefore) + this.mRadius;
            float longOffset = ((float) longPaddingBefore) + this.mRadius;
            if (this.mCentered) {
                longOffset += (((float) ((longSize - longPaddingBefore) - longPaddingAfter)) / 2
                .0f) - ((((float) count) * threeRadius) / 2.0f);
            }
    ...
    private int measureLong(int measureSpec) {
        ...
        int count = this.mViewPager.getAdapter().getCount();
        int result = (int) (((((float) (getPaddingLeft() + getPaddingRight())) + (((float) (count
                * 2)) * this.mRadius)) + (((float) (count - 1)) * this.mRadius)) + 1.0f);
