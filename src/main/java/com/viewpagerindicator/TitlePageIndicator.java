...
public class TitlePageIndicator extends View implements PageIndicator {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.mViewPager != null) {
            ...
            if (count != 0) {
                ...
                if (this.mCurrentPage > 0) {
                    for (i = this.mCurrentPage - 1; i >= 0; i--) {
                        ...
                        if (((float) bound.left) < leftClip) {
                            ...
                            if (((float) bound.right) + this.mTitlePadding > ((float) rightBound
                                    .left)) {
                                bound.left = (int) (((float) (rightBound.left - w)) - this
                                        .mTitlePadding);
                ...
                if (this.mCurrentPage < countMinusOne) {
                    for (i = this.mCurrentPage + 1; i < count; i++) {
                        ...
                        if (((float) bound.right) > rightClip) {
                            ...
                            if (((float) bound.left) - this.mTitlePadding < ((float) leftBound
                                    .right)) {
                ...
                while (i < count) {
                    bound = (Rect) bounds.get(i);
                    if ((bound.left > left && bound.left < right) || (bound.right > left && bound
                            .right < right)) {
                        ...
                        if (currentPage && currentSelected) {
                            this.mPaintText.setAlpha(colorTextAlpha - ((int) (((float)
                                    colorTextAlpha) * selectedPercent)));
                        }
                        if (i < boundsSize - 1) {
                            rightBound = (Rect) bounds.get(i + 1);
                            if (((float) bound.right) + this.mTitlePadding > ((float) rightBound
                        ...
                        canvas.drawText(pageTitle, 0, pageTitle.length(), (float) bound.left,
                                this.mTopPadding + ((float) bound.bottom), this.mPaintText);
                        if (currentPage && currentSelected) {
                            this.mPaintText.setColor(this.mColorSelected);
                            this.mPaintText.setAlpha((int) (((float) (this.mColorSelected >>> 24)
                            ) * selectedPercent));
                            canvas.drawText(pageTitle, 0, pageTitle.length(), (float) bound.left,
                                    this.mTopPadding + ((float) bound.bottom), this.mPaintText);
                        }
                ...
                switch (this.mFooterIndicatorStyle) {
                    ...
                    case Underline:
                        if (currentSelected && page < boundsSize) {
                            ...
                            float rightPlusPadding = ((float) underlineBounds.right) + this
                                    .mFooterIndicatorUnderlinePadding;
                            float leftMinusPadding = ((float) underlineBounds.left) - this
                                    .mFooterIndicatorUnderlinePadding;
    ...
    private ArrayList<Rect> calculateAllBounds(Paint paint) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            bounds.left = (int) ((((float) halfWidth) - (((float) w) / 2.0f)) + ((((float) (i -
                    this.mCurrentPage)) - this.mPageOffset) * ((float) width)));
