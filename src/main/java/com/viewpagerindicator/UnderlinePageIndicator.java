...
public class UnderlinePageIndicator extends View implements PageIndicator {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.mViewPager != null) {
            ...
            float pageWidth = ((float) ((getWidth() - paddingLeft) - getPaddingRight())) / (1.0f
                    * ((float) count));
            float left = ((float) paddingLeft) + ((((float) this.mCurrentPage) + this
                    .mPositionOffset) * pageWidth);
            Canvas canvas2 = canvas;
            canvas2.drawRect(left, (float) getPaddingTop(), left + pageWidth, (float) (getHeight
                    () - getPaddingBottom()), this.mPaint);
        }
    }
