...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements
    ...
    public void addView(View child, int index, LayoutParams params) {
        ...
        throw new UnsupportedOperationException("Refreshable View is not a ViewGroup so can't " +
                "addView");
    }
    ...
    public final boolean isPullToRefreshOverScrollEnabled() {
        return VERSION.SDK_INT >= 9 && this.mOverScrollEnabled && OverscrollHelper
                ...
                case 2:
                    if (this.mScrollingWhileRefreshingEnabled || !isRefreshing()) {
                        if (isReadyForPull()) {
                            ...
                            if (absDiff > ((float) this.mTouchSlop) && (!this.mFilterTouchEvents
                                    || absDiff > Math.abs(oppositeDiff))) {
                                if (!this.mMode.showHeaderLoadingLayout() || diff < 1.0f ||
                                        !isReadyForPullStart()) {
                                    if (this.mMode.showFooterLoadingLayout() && diff <= -1.0f &&
                                            isReadyForPullEnd()) {
    ...
    protected final void onRestoreInstanceState(Parcelable state) {
        if (state instanceof Bundle) {
            ...
            this.mScrollingWhileRefreshingEnabled = bundle.getBoolean
    ...
    protected final Parcelable onSaveInstanceState() {
        ...
        bundle.putBoolean(STATE_SCROLLING_REFRESHING_ENABLED, this
    ...
    protected final void refreshRefreshableViewSize(int width, int height) {
        LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) this.mRefreshableViewWrapper
                .getLayoutParams();
        switch (getPullToRefreshScrollDirection()) {
            case HORIZONTAL:
                if (lp.width != width) {
    ...
    private void pullEvent() {
        ...
        switch (this.mCurrentMode) {
            case PULL_FROM_END:
                newScrollValue = Math.round(Math.max(initialMotionValue - lastMotionValue, 0.0f)
                        / FRICTION);
                itemDimension = getFooterSize();
                break;
            default:
                newScrollValue = Math.round(Math.min(initialMotionValue - lastMotionValue, 0.0f)
                        / FRICTION);
        ...
        if (newScrollValue != 0 && !isRefreshing()) {
            ...
            } else if (this.mState == State.PULL_TO_REFRESH && itemDimension < Math.abs
                    (newScrollValue)) {
