...
public class AnimatorInflater {
    private static final int[] Animator                      = new int[]{16843073, 16843160,
            16843198, 16843199, 16843200, 16843486, 16843487, 16843488};
    ...
    public static Animator loadAnimator(Context context, int id) throws NotFoundException {
        ...
        try {
            ...
            rnf = new NotFoundException("Can't load animation resource ID #0x" + Integer
                    .toHexString(id));
            rnf.initCause(ex);
            throw rnf;
    ...
    private static Animator createAnimatorFromXml(Context c, XmlPullParser parser) throws
            XmlPullParserException, IOException {
        return createAnimatorFromXml(c, parser, Xml.asAttributeSet(parser), null, 0);
    }

    private static Animator createAnimatorFromXml(Context c, XmlPullParser parser, AttributeSet
            attrs, AnimatorSet parent, int sequenceOrdering) throws XmlPullParserException,
            IOException {
        Animator anim = null;
            ...
            if ((type != 3 || parser.getDepth() > depth) && type != 1) {
                if (type == 2) {
                    ...
                    if (name.equals("objectAnimator")) {
                        ...
                        createAnimatorFromXml(c, parser, attrs, (AnimatorSet) anim, orderingValue
                                .type == 16 ? orderingValue.data : 0);
                        a.recycle();
                    } else {
    ...
    private static ObjectAnimator loadObjectAnimator(Context context, AttributeSet attrs) throws
            NotFoundException {
    ...
    private static ValueAnimator loadAnimator(Context context, AttributeSet attrs, ValueAnimator
        ...
        if ((hasFrom && fromType >= 28 && fromType <= 31) || (hasTo && toType >= 28 && toType <=
                31)) {
            getFloats = false;
        ...
        if (getFloats) {
            ...
            if (hasFrom) {
                ...
                if (hasTo) {
                    ...
                    anim.setFloatValues(new float[]{valueFrom, valueTo});
                } else {
                    anim.setFloatValues(new float[]{valueFrom});
                }
            } else {
                if (toType == 5) {
                    valueTo = a.getDimension(6, 0.0f);
                ...
                anim.setFloatValues(new float[]{valueTo});
            }
        } else if (hasFrom) {
            int valueFrom2;
            if (fromType == 5) {
            ...
            if (hasTo) {
                ...
                anim.setIntValues(new int[]{valueFrom2, valueTo});
            } else {
                anim.setIntValues(new int[]{valueFrom2});
            }
        } else if (hasTo) {
            if (toType == 5) {
                valueTo = (int) a.getDimension(6, 0.0f);
            ...
            anim.setIntValues(new int[]{valueTo});
        }
