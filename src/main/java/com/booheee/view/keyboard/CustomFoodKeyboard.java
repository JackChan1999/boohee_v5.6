...
public class CustomFoodKeyboard extends FrameLayout {
    ...
    private OnCustomFoodResultListener onResultListener;
    ...
    private class NumBtnListener implements OnClickListener {
        ...
        public void onClick(View v) {
            if (v.getId() == R.id.btn_dot) {
                if (!(CustomFoodKeyboard.this.hasDot || CustomFoodKeyboard.this.inputBuffer
                        .toString().equals(""))) {
                    CustomFoodKeyboard.this.inputBuffer.append(".");
                    CustomFoodKeyboard.this.hasDot = true;
                }
            } else if (v.getId() == R.id.btn_back) {
                CustomFoodKeyboard.this.doBack();
            } else {
                CustomFoodKeyboard.this.doNum(v);
            }
            if (TextUtils.isEmpty(CustomFoodKeyboard.this.inputBuffer.toString())) {
                CustomFoodKeyboard.this.mAmount = 0.0f;
                CustomFoodKeyboard.this.mCalory = 0.0f;
            } else {
                CustomFoodKeyboard.this.mAmount = Float.parseFloat(CustomFoodKeyboard.this
                        .inputBuffer.toString());
                CustomFoodKeyboard.this.mCalory = (float) CustomFoodKeyboard.this.calcCalory
                        (CustomFoodKeyboard.this.mAmount);
            }
            CustomFoodKeyboard.this.txt_value.setText(CustomFoodKeyboard.this.inputBuffer
                    .toString());
            CustomFoodKeyboard.this.txt_calory.setText(String.format("%d 千卡", new
                    Object[]{Integer.valueOf((int) CustomFoodKeyboard.this.mCalory)}));
            CustomFoodKeyboard.this.onResultListener.onValue(CustomFoodKeyboard.this.mAmount,
                    CustomFoodKeyboard.this.mCalory);
        }
    }

    public CustomFoodKeyboard(Context context) {
        this(context, null);
    }

    public CustomFoodKeyboard(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CustomFoodKeyboard(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setOnResultListener(OnCustomFoodResultListener onResultListener) {
