...
import com.boohee.myview.ViewPagerHeaderScroll.widget.SlidingTabLayout.TabColorizer;

import uk.co.senab.photoview.IPhotoView;

class SlidingTabStrip extends LinearLayout {
    private static final int DEFAULT_BOTTOM_BORDER_THICKNESS_DIPS = 0;
    private static final int DEFAULT_SELECTED_INDICATOR_COLOR     = -177592;
    ...
    SlidingTabStrip(Context context, AttributeSet attrs) {
        ...
        this.mSelectedIndicatorThickness = (int) (IPhotoView.DEFAULT_MAX_SCALE * density);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        TabColorizer tabColorizer = this.mCustomTabColorizer != null ? this.mCustomTabColorizer :
        ...
        if (childCount > 0) {
            ...
            if (this.mSelectionOffset > 0.0f && this.mSelectedPosition < getChildCount() - 1) {
                ...
                left = (int) ((this.mSelectionOffset * ((float) nextTitle.getLeft())) + ((1.0f -
                        this.mSelectionOffset) * ((float) left)));
                right = (int) ((this.mSelectionOffset * ((float) nextTitle.getRight())) + ((1.0f
                        - this.mSelectionOffset) * ((float) right)));
            }
            this.mSelectedIndicatorPaint.setColor(color);
            canvas.drawRect((float) left, (float) (height - this.mSelectedIndicatorThickness),
                    (float) right, (float) height, this.mSelectedIndicatorPaint);
        }
        canvas.drawRect(0.0f, (float) (height - this.mBottomBorderThickness), (float) getWidth(),
                (float) height, this.mBottomBorderPaint);
    }
    ...
    private static int blendColors(int color1, int color2, float ratio) {
        ...
        return Color.rgb((int) ((((float) Color.red(color1)) * ratio) + (((float) Color.red
                (color2)) * inverseRation)), (int) ((((float) Color.green(color1)) * ratio) + ((
                (float) Color.green(color2)) * inverseRation)), (int) ((((float) Color.blue(color1)) * ratio) + (((float) Color.blue(color2)) * inverseRation)));
    }
}
