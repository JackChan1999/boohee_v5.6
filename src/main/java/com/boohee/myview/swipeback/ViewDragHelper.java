...
public class ViewDragHelper {
    ...
    public void setSensitivity(Context context, float sensitivity) {
        this.mTouchSlop = (int) (((float) ViewConfiguration.get(context).getScaledTouchSlop()) *
                (1.0f / Math.max(0.0f, Math.min(1.0f, sensitivity))));
    }
    ...
    public void captureChildView(View childView, int activePointerId) {
        if (childView.getParent() != this.mParentView) {
            throw new IllegalArgumentException("captureChildView: parameter must be a descendant " +
                    "of the ViewDragHelper's tracked parent view (" + this.mParentView +
    ...
    public void abort() {
        ...
        if (this.mDragState == 2) {
            ...
            this.mCallback.onViewPositionChanged(this.mCapturedView, newX, newY, newX - oldX,
                    newY - oldY);
        }
        setDragState(0);
    }
    ...
    public boolean settleCapturedViewAt(int finalLeft, int finalTop) {
        if (this.mReleaseInProgress) {
            return forceSettleCapturedViewAt(finalLeft, finalTop, (int) VelocityTrackerCompat
                    .getXVelocity(this.mVelocityTracker, this.mActivePointerId), (int)
                    VelocityTrackerCompat.getYVelocity(this.mVelocityTracker, this
                            .mActivePointerId));
        }
    ...
    private boolean forceSettleCapturedViewAt(int finalLeft, int finalTop, int xvel, int yvel) {
        ...
        this.mScroller.startScroll(startLeft, startTop, dx, dy, computeSettleDuration(this
                .mCapturedView, dx, dy, xvel, yvel));
        setDragState(2);
        return true;
    }
    ...
    private int computeSettleDuration(View child, int dx, int dy, int xvel, int yvel) {
        ...
        return (int) ((((float) computeAxisDuration(dx, xvel, this.mCallback
                .getViewHorizontalDragRange(child))) * (xvel != 0 ? ((float) absXVel) / ((float)
                addedVel) : ((float) absDx) / ((float) addedDistance))) + (((float)
                computeAxisDuration(dy, yvel, this.mCallback.getViewVerticalDragRange(child))) *
                (yvel != 0 ? ((float) absYVel) / ((float) addedVel) : ((float) absDy) / ((float)
                        addedDistance))));
    }
    ...
    private int computeAxisDuration(int delta, int velocity, int motionRange) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) *
                distanceInfluenceForSnapDuration(Math.min(1.0f, ((float) Math.abs(delta)) / (
                        (float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) (((((float) Math.abs(delta)) / ((float) motionRange)) + 1.0f) * 256
            .0f);
        }
    ...
    public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
        if (this.mReleaseInProgress) {
            this.mScroller.fling(this.mCapturedView.getLeft(), this.mCapturedView.getTop(), (int)
                    VelocityTrackerCompat.getXVelocity(this.mVelocityTracker, this
                            .mActivePointerId), (int) VelocityTrackerCompat.getYVelocity(this
                    .mVelocityTracker, this.mActivePointerId), minLeft, maxLeft, minTop, maxTop);
            setDragState(2);
            return;
        }
    ...
    private void clearMotionHistory(int pointerId) {
        if (this.mInitialMotionX != null) {
            ...
            this.mInitialEdgeTouched[pointerId] = 0;
    ...
    private void ensureMotionHistorySizeForId(int pointerId) {
        if (this.mInitialMotionX == null || this.mInitialMotionX.length <= pointerId) {
            ...
            if (this.mInitialMotionX != null) {
                ...
                System.arraycopy(this.mInitialEdgeTouched, 0, iit, 0, this.mInitialEdgeTouched
                        .length);
                System.arraycopy(this.mEdgeDragsInProgress, 0, edip, 0, this.mEdgeDragsInProgress
                        .length);
    ...
    private void saveInitialMotion(float x, float y, int pointerId) {
        ...
        fArr[pointerId] = y;
        this.mInitialEdgeTouched[pointerId] = getEdgeTouched((int) x, (int) y);
    ...
    protected boolean canScroll(View v, boolean checkV, int dx, int dy, int x, int y) {
        if (v instanceof ViewGroup) {
            ...
            for (int i = group.getChildCount() - 1; i >= 0; i--) {
                ...
                if (x + scrollX >= child.getLeft() && x + scrollX < child.getRight() && y +
                        scrollY >= child.getTop() && y + scrollY < child.getBottom()) {
                    if (canScroll(child, true, dx, dy, (x + scrollX) - child.getLeft(), (y +
        ...
        return checkV && (ViewCompat.canScrollHorizontally(v, -dx) || ViewCompat
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case 0:
                ...
                edgesTouched = this.mInitialEdgeTouched[pointerId];
    ...
    L_0x02b8:
        ...
        goto L_0x002f;
        */
        throw new UnsupportedOperationException("Method not decompiled: com.boohee.myview" +
                ".swipeback.ViewDragHelper.processTouchEvent(android.view.MotionEvent):void");
    }
    ...
    private boolean checkNewEdgeDrag(float delta, float odelta, int pointerId, int edge) {
        ...
        if ((this.mInitialEdgeTouched[pointerId] & edge) != edge || (this.mTrackingEdges & edge)
                == 0 || (this.mEdgeDragsLocked[pointerId] & edge) == edge || (this
        ...
        if (absDelta < 0.5f * absODelta && this.mCallback.onEdgeLock(edge)) {
            ...
            return false;
        } else if ((this.mEdgeDragsInProgress[pointerId] & edge) != 0 || absDelta <= ((float)
                this.mTouchSlop)) {
            return false;
        } else {
            return true;
    ...
    public boolean isEdgeTouched(int edges) {
        int count = this.mInitialEdgeTouched.length;
        for (int i = 0; i < count; i++) {
    ...
    public boolean isEdgeTouched(int edges, int pointerId) {
        return isPointerDown(pointerId) && (this.mInitialEdgeTouched[pointerId] & edges) != 0;
    }

    private void releaseViewForPointerUp() {
        this.mVelocityTracker.computeCurrentVelocity(1000, this.mMaxVelocity);
        dispatchViewReleased(clampMag(VelocityTrackerCompat.getXVelocity(this.mVelocityTracker,
                this.mActivePointerId), this.mMinVelocity, this.mMaxVelocity), clampMag
                (VelocityTrackerCompat.getYVelocity(this.mVelocityTracker, this.mActivePointerId)
    ...
    private void dragTo(int left, int top, int dx, int dy) {
        ...
        if (dx != 0 || dy != 0) {
            this.mCallback.onViewPositionChanged(this.mCapturedView, clampedX, clampedY, clampedX
                    - oldLeft, clampedY - oldTop);
        }
    }
    ...
    public View findTopChildUnder(int x, int y) {
        for (int i = this.mParentView.getChildCount() - 1; i >= 0; i--) {
            ...
            if (x >= child.getLeft() && x < child.getRight() && y >= child.getTop() && y < child
                    .getBottom()) {
                return child;
            }
        }
