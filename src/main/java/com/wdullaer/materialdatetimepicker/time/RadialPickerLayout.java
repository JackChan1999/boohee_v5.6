...
public class RadialPickerLayout extends FrameLayout implements OnTouchListener {
    ...
    public void initialize(Context context, TimePickerDialog timePickerDialog, int
            initialHoursOfDay, int initialMinutes, boolean is24HourMode) {
        if (this.mTimeInitialized) {
            Log.e(TAG, "Time has already been initialized.");
            return;
        }
        int i;
        this.mController = timePickerDialog;
        ...
        if (!this.mIs24HourMode) {
            this.mAmPmCirclesView.initialize(context, this.mController, initialHoursOfDay < 12 ?
                    0 : 1);
        ...
        for (int i2 = 0; i2 < 12; i2++) {
            hoursTexts[i2] = is24HourMode ? String.format("%02d", new Object[]{Integer.valueOf
                    (iArr2[i2])}) : String.format("%d", new Object[]{Integer.valueOf(iArr[i2])});
            innerHoursTexts[i2] = String.format("%d", new Object[]{Integer.valueOf(iArr[i2])});
            minutesTexts[i2] = String.format("%02d", new Object[]{Integer.valueOf(iArr3[i2])});
        }
        this.mHourRadialTextsView.initialize(res, hoursTexts, is24HourMode ? innerHoursTexts :
                null, this.mController, true);
        RadialTextsView radialTextsView = this.mHourRadialTextsView;
        if (is24HourMode) {
            i = initialHoursOfDay;
        } else {
            i = iArr[initialHoursOfDay % 12];
        }
        radialTextsView.setSelection(i);
        this.mHourRadialTextsView.invalidate();
        this.mMinuteRadialTextsView.initialize(res, minutesTexts, null, this.mController, false);
        ...
        this.mHourRadialSelectorView.initialize(context, this.mController, is24HourMode, true,
                (initialHoursOfDay % 12) * 30, isHourInnerCircle(initialHoursOfDay));
        this.mMinuteRadialSelectorView.initialize(context, this.mController, false, false,
                initialMinutes * 6, false);
    ...
    private void setItem(int index, int value) {
        if (index == 0) {
            ...
            this.mHourRadialSelectorView.setSelection((value % 12) * 30, isHourInnerCircle(value)
                    , false);
    ...
    private int reselectSelector(int degrees, boolean isInnerCircle, boolean forceToVisibleValue,
    ...
    private int getDegreesFromCoords(float pointX, float pointY, boolean forceLegal, Boolean[]
            isInnerCircle) {
        int currentItem = getCurrentItemShowing();
        if (currentItem == 0) {
            return this.mHourRadialSelectorView.getDegreesFromCoords(pointX, pointY, forceLegal,
                    isInnerCircle);
        }
        if (currentItem == 1) {
            return this.mMinuteRadialSelectorView.getDegreesFromCoords(pointX, pointY,
        ...
        switch (event.getAction()) {
            case 0:
                ...
                if (this.mIs24HourMode) {
                    ...
                    this.mIsTouchingAmOrPm = this.mAmPmCirclesView.getIsTouchingAmOrPm(eventX,
                            eventY);
                }
                if (this.mIsTouchingAmOrPm == 0 || this.mIsTouchingAmOrPm == 1) {
                    ...
                    this.mHandler.postDelayed(new Runnable() {
                        public void run() {
                            RadialPickerLayout.this.mAmPmCirclesView.setAmOrPmPressed
                                    (RadialPickerLayout.this.mIsTouchingAmOrPm);
                            RadialPickerLayout.this.mAmPmCirclesView.invalidate();
                        }
                    }, (long) this.TAP_TIMEOUT);
                } else {
                    this.mDownDegrees = getDegreesFromCoords(eventX, eventY, this
                            .mAccessibilityManager.isTouchExplorationEnabled(), isInnerCircle);
                    if (this.mDownDegrees != -1) {
                        this.mController.tryVibrate();
                        this.mHandler.postDelayed(new Runnable() {
                            public void run() {
                                RadialPickerLayout.this.mDoingMove = true;
                                int value = RadialPickerLayout.this.reselectSelector
                                        (RadialPickerLayout.this.mDownDegrees, isInnerCircle[0]
                                                .booleanValue(), false, true);
                                RadialPickerLayout.this.mLastValueSelected = value;
                                RadialPickerLayout.this.mListener.onValueSelected
                                        (RadialPickerLayout.this.getCurrentItemShowing(), value,
                                                false);
                            }
                        }, (long) this.TAP_TIMEOUT);
                    }
            ...
            case 1:
                if (this.mInputEnabled) {
                    ...
                    if (this.mIsTouchingAmOrPm == 0 || this.mIsTouchingAmOrPm == 1) {
                        int isTouchingAmOrPm = this.mAmPmCirclesView.getIsTouchingAmOrPm(eventX,
                                eventY);
                    ...
                    if (this.mDownDegrees != -1) {
                        degrees = getDegreesFromCoords(eventX, eventY, this.mDoingMove,
                                isInnerCircle);
                        if (degrees != -1) {
                            value = reselectSelector(degrees, isInnerCircle[0].booleanValue(),
                                    !this.mDoingMove, false);
            ...
            case 2:
                if (this.mInputEnabled) {
                    ...
                    if (this.mDoingMove || dX > ((float) this.TOUCH_SLOP) || dY > ((float) this
                            .TOUCH_SLOP)) {
                        if (this.mIsTouchingAmOrPm == 0 || this.mIsTouchingAmOrPm == 1) {
                            this.mHandler.removeCallbacksAndMessages(null);
                            if (this.mAmPmCirclesView.getIsTouchingAmOrPm(eventX, eventY) != this
                                    .mIsTouchingAmOrPm) {
                                this.mAmPmCirclesView.setAmOrPmPressed(-1);
                                this.mAmPmCirclesView.invalidate();
                                this.mIsTouchingAmOrPm = -1;
