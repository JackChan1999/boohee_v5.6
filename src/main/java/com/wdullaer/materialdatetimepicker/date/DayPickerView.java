...
public abstract class DayPickerView extends ListView implements OnScrollListener,
    ...
    protected class ScrollStateRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (Log.isLoggable(DayPickerView.TAG, 3)) {
                Log.d(DayPickerView.TAG, "new scroll state: " + this.mNewState + " old state: " +
                        DayPickerView.this.mPreviousScrollState);
            }
            if (this.mNewState != 0 || DayPickerView.this.mPreviousScrollState == 0 ||
                    DayPickerView.this.mPreviousScrollState == 1) {
    ...
    public abstract MonthAdapter createMonthAdapter(Context context, DatePickerController
    ...
    public boolean goTo(CalendarDay day, boolean animate, boolean setSelected, boolean
            forceScroll) {
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int
        ...
        if (child != null) {
            this.mPreviousScrollPosition = (long) ((view.getFirstVisiblePosition() * child
    ...
    private boolean restoreAccessibilityFocus(CalendarDay day) {
        ...
        for (int i = 0; i < childCount; i++) {
            View child = getChildAt(i);
            if ((child instanceof MonthView) && ((MonthView) child).restoreAccessibilityFocus
                    (day)) {
                return true;
            }
        }
