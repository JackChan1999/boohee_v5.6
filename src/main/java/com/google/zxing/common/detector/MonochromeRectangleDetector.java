...
public final class MonochromeRectangleDetector {
    ...
    private ResultPoint findCornerFromCenter(int centerX, int deltaX, int left, int right, int
            centerY, int deltaY, int top, int bottom, int maxWhiteRun) throws NotFoundException {
        int[] lastRange = null;
        int y = centerY;
        int x = centerX;
        while (y < bottom && y >= top && x < right && x >= left) {
            ...
            if (range != null) {
                ...
                return new ResultPoint((float) lastX, deltaX < 0 ? (float) lastRange[0] : (float)
                        lastRange[1]);
            }
        }
        throw NotFoundException.getNotFoundInstance();
    ...
    L_0x0082:
        ...
        goto L_0x0078;
        */
        throw new UnsupportedOperationException("Method not decompiled: com.google.zxing.common" +
                ".detector.MonochromeRectangleDetector.blackWhiteRange(int, int, int, int, " +
                "boolean):int[]");
    }
}
