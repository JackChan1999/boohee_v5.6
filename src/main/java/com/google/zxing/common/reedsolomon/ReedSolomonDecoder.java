...
public final class ReedSolomonDecoder {
    ...
    public void decode(int[] received, int twoS) throws ReedSolomonException {
        ...
        if (!noError) {
            ...
            GenericGFPoly[] sigmaOmega = runEuclideanAlgorithm(this.field.buildMonomial(twoS, 1),
                    syndrome, twoS);
    ...
    private GenericGFPoly[] runEuclideanAlgorithm(GenericGFPoly a, GenericGFPoly b, int R) throws
        ...
        for (int i = 0; i < s; i++) {
            ...
            for (int j = 0; j < s; j++) {
                if (i != j) {
                    ...
                    denominator = this.field.multiply(denominator, (term & 1) == 0 ? term | 1 :
                            term & -2);
                }
            }
            result[i] = this.field.multiply(errorEvaluator.evaluateAt(xiInverse), this.field
                    .inverse(denominator));
