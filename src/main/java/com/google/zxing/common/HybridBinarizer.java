...
public final class HybridBinarizer extends GlobalHistogramBinarizer {
    ...
    private static void calculateThresholdForBlock(byte[] luminances, int subWidth, int
            subHeight, int width, int height, int[][] blackPoints, BitMatrix matrix) {
    ...
    private static void thresholdBlock(byte[] luminances, int xoffset, int yoffset, int
            threshold, int stride, BitMatrix matrix) {
        int y = 0;
    ...
    private static int[][] calculateBlackPoints(byte[] luminances, int subWidth, int subHeight,
        ...
        int[][] blackPoints = (int[][]) Array.newInstance(Integer.TYPE, new int[]{subHeight,
                subWidth});
        for (int y = 0; y < subHeight; y++) {
            int yoffset = y << 3;
            int maxYOffset = height - 8;
                ...
                if (max - min <= 24) {
                    ...
                    if (y > 0 && x > 0) {
                        int averageNeighborBlackPoint = ((blackPoints[y - 1][x] +
                                (blackPoints[y][x - 1] * 2)) + blackPoints[y - 1][x - 1]) / 4;
