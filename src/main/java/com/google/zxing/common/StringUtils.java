...
public final class StringUtils {
    ...
    static {
        boolean z = SHIFT_JIS.equalsIgnoreCase(PLATFORM_DEFAULT_ENCODING) || EUC_JP
    ...
    public static String guessEncoding(byte[] bytes, Map<DecodeHintType, ?> hints) {
        ...
        int isoHighOther = 0;
        boolean utf8bom = bytes.length > 3 && bytes[0] == (byte) -17 && bytes[1] == (byte) -69 &&
                bytes[2] == (byte) -65;
        ...
        if (canBeShiftJIS && (ASSUME_SHIFT_JIS || sjisMaxKatakanaWordLength >= 3 ||
                sjisMaxDoubleBytesWordLength >= 3)) {
            return SHIFT_JIS;
        }
        if (canBeISO88591 && canBeShiftJIS) {
            String str = (!(sjisMaxKatakanaWordLength == 2 && sjisKatakanaChars == 2) &&
                    isoHighOther * 10 < length) ? ISO88591 : SHIFT_JIS;
            return str;
