...
public final class Detector {
    ...
    private static ResultPoint[] findVertices(BitMatrix matrix, int startRow, int startColumn) {
        ...
        copyToResult(result, findRowsWithPattern(matrix, height, width, startRow, startColumn,
                START_PATTERN), INDEXES_START_PATTERN);
        if (result[4] != null) {
            startColumn = (int) result[4].getX();
            startRow = (int) result[4].getY();
        }
        copyToResult(result, findRowsWithPattern(matrix, height, width, startRow, startColumn,
                STOP_PATTERN), INDEXES_STOP_PATTERN);
        return result;
    }

    private static void copyToResult(ResultPoint[] result, ResultPoint[] tmpResult, int[]
            destinationIndexes) {
    ...
    private static ResultPoint[] findRowsWithPattern(BitMatrix matrix, int height, int width, int
            startRow, int startColumn, int[] pattern) {
        int stopRow;
        ResultPoint[] result = new ResultPoint[4];
        boolean found = false;
        int[] counters = new int[pattern.length];
        while (startRow < height) {
            int[] loc = findGuardPattern(matrix, startColumn, startRow, width, false, pattern,
                    counters);
            int[] previousRowLoc;
            int skippedRowCount;
            int i;
            if (loc != null) {
                while (startRow > 0) {
                    startRow--;
                    previousRowLoc = findGuardPattern(matrix, startColumn, startRow, width,
                            false, pattern, counters);
                ...
                if (found) {
                    ...
                    while (stopRow < height) {
                        loc = findGuardPattern(matrix, previousRowLoc[0], stopRow, width, false,
                                pattern, counters);
                        if (loc == null && Math.abs(previousRowLoc[0] - loc[0]) < 5 && Math.abs
                                (previousRowLoc[1] - loc[1]) < 5) {
                            previousRowLoc = loc;
                            skippedRowCount = 0;
        ...
        if (found) {
            ...
            previousRowLoc = new int[]{(int) result[0].getX(), (int) result[1].getX()};
            while (stopRow < height) {
                loc = findGuardPattern(matrix, previousRowLoc[0], stopRow, width, false, pattern,
                        counters);
                if (loc == null) {
                }
    ...
    private static int[] findGuardPattern(BitMatrix matrix, int column, int row, int width,
        ...
        for (int x = patternStart; x < width; x++) {
            if ((matrix.get(x, row) ^ isWhite) != 0) {
                ...
                if (counterPosition != patternLength - 1) {
                    counterPosition++;
                } else if (patternMatchVariance(counters, pattern, MAX_INDIVIDUAL_VARIANCE) <
                        MAX_AVG_VARIANCE) {
        ...
        if (counterPosition != patternLength - 1 || patternMatchVariance(counters, pattern,
    ...
    private static float patternMatchVariance(int[] counters, int[] pattern, float
        ...
        for (int x = 0; x < numCounters; x++) {
            ...
            float variance = ((float) counter) > scaledPattern ? ((float) counter) -
                    scaledPattern : scaledPattern - ((float) counter);
