...
final class DetectionResultRowIndicatorColumn extends DetectionResultColumn {
    ...
    int adjustCompleteIndicatorColumnRowNumbers(BarcodeMetadata barcodeMetadata) {
        ...
        ResultPoint bottom = this.isLeft ? boundingBox.getBottomLeft() : boundingBox
                .getBottomRight();
        int firstRow = imageRowToCodewordIndex((int) top.getY());
        int lastRow = imageRowToCodewordIndex((int) bottom.getY());
        float averageRowHeight = ((float) (lastRow - firstRow)) / ((float) barcodeMetadata
                .getRowCount());
        int barcodeRow = -1;
        int maxRowHeight = 1;
        ...
        while (codewordsRow < lastRow) {
            if (codewords[codewordsRow] != null) {
                ...
                } else if (rowDifference < 0 || codeword.getRowNumber() >= barcodeMetadata
                        .getRowCount() || rowDifference > codewordsRow) {
    ...
    BarcodeMetadata getBarcodeMetadata() {
        ...
        if (barcodeColumnCount.getValue().length == 0 || barcodeRowCountUpperPart.getValue()
                .length == 0 || barcodeRowCountLowerPart.getValue().length == 0 || barcodeECLevel
                .getValue().length == 0 || barcodeColumnCount.getValue()[0] < 1 ||
                barcodeRowCountUpperPart.getValue()[0] + barcodeRowCountLowerPart.getValue()[0] <
                        3 || barcodeRowCountUpperPart.getValue()[0] + barcodeRowCountLowerPart
                .getValue()[0] > 90) {
            return null;
        }
        ...
        for (int codewordRow = 0; codewordRow < codewords.length; codewordRow++) {
            ...
            if (codewords[codewordRow] != null) {
                ...
                if (codewordRowNumber <= barcodeMetadata.getRowCount()) {
                    ...
                    switch (codewordRowNumber % 3) {
                        case 0:
                            if ((rowIndicatorValue * 3) + 1 == barcodeMetadata
                        ...
                        case 1:
                            if (rowIndicatorValue / 3 != barcodeMetadata.getErrorCorrectionLevel
                                    () || rowIndicatorValue % 3 != barcodeMetadata
