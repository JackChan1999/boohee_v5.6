...
final class PDF417CodewordDecoder {
    private static final float[][] RATIOS_TABLE = ((float[][]) Array.newInstance(Float.TYPE, new
            int[]{PDF417Common.SYMBOL_TABLE.length, 8}));
    ...
    private static int[] sampleBitCounts(int[] moduleBitCount) {
        ...
        for (int i = 0; i < 17; i++) {
            if (((float) (moduleBitCount[bitCountIndex] + sumPreviousBits)) <= (bitCountSum / 34
            .0f) + ((((float) i) * bitCountSum) / 17.0f)) {
