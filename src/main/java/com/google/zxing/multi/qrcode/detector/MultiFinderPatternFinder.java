...
final class MultiFinderPatternFinder extends FinderPatternFinder {
    ...
    private static final class ModuleSizeComparator implements Comparator<FinderPattern>,
            Serializable {
    ...
    public FinderPatternInfo[] findMulti(Map<DecodeHintType, ?> hints) throws NotFoundException {
            ...
            if (hints.containsKey(DecodeHintType.TRY_HARDER)) {
                ...
                if (hints != null) {
                    if (hints.containsKey(DecodeHintType.PURE_BARCODE)) {
                        ...
                        maxJ = image.getWidth();
                        iSkip = (int) ((((float) maxI) / 228.0f) * IPhotoView.DEFAULT_MAX_SCALE);
                        ...
                        while (i < maxI) {
                            ...
                            while (j < maxJ) {
                                if (!image.get(j, i)) {
                                    ...
                                    currentState++;
                                    stateCount[currentState] = stateCount[currentState] + 1;
                                } else if (foundPatternCross(stateCount) || !handlePossibleCenter
                        ...
                        if (result.isEmpty()) {
                            return (FinderPatternInfo[]) result.toArray(new
                ...
                for (FinderPattern[] pattern2 : patternInfo) {
                    ...
                    result.add(new FinderPatternInfo(pattern2));
                }
                if (result.isEmpty()) {
        ...
        if (hints != null) {
            if (hints.containsKey(DecodeHintType.PURE_BARCODE)) {
                ...
                patternInfo = selectMutipleBestPatterns();
                result = new ArrayList();
                for (FinderPattern[] pattern22 : patternInfo) {
                    ResultPoint.orderBestPatterns(pattern22);
                    result.add(new FinderPatternInfo(pattern22));
                }
                if (result.isEmpty()) {
                    return EMPTY_RESULT_ARRAY;
                }
        ...
        patternInfo = selectMutipleBestPatterns();
        result = new ArrayList();
        for (FinderPattern[] pattern222 : patternInfo) {
            ResultPoint.orderBestPatterns(pattern222);
            result.add(new FinderPatternInfo(pattern222));
        }
        if (result.isEmpty()) {
            return (FinderPatternInfo[]) result.toArray(new FinderPatternInfo[result.size()]);
        }
