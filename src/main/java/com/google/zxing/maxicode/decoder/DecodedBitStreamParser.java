...
final class DecodedBitStreamParser {
    ...
    private static final String[]     SETS         = new
    ...
    private static int getCountry(byte[] bytes) {
        return getInt(bytes, new byte[]{(byte) 53, (byte) 54, (byte) 43, (byte) 44, (byte) 45,
                (byte) 46, (byte) 47, (byte) 48, (byte) 37, (byte) 38});
    }

    private static int getServiceClass(byte[] bytes) {
        return getInt(bytes, new byte[]{(byte) 55, (byte) 56, (byte) 57, (byte) 58, (byte) 59,
                (byte) 60, (byte) 49, (byte) 50, (byte) 51, (byte) 52});
    }

    private static int getPostCode2Length(byte[] bytes) {
        return getInt(bytes, new byte[]{(byte) 39, (byte) 40, (byte) 41, (byte) 42, (byte) 31,
                (byte) 32});
    }

    private static int getPostCode2(byte[] bytes) {
        return getInt(bytes, new byte[]{(byte) 33, (byte) 34, (byte) 35, (byte) 36, (byte) 25,
                JSONLexer.EOI, (byte) 27, (byte) 28, (byte) 29, (byte) 30, (byte) 19, (byte) 20,
                (byte) 21, (byte) 22, (byte) 23, (byte) 24, (byte) 13, df.l, df.m, df.n, (byte)
                17, (byte) 18, (byte) 7, (byte) 8, (byte) 9, (byte) 10, (byte) 11, (byte) 12,
                (byte) 1, (byte) 2});
    }

    private static String getPostCode3(byte[] bytes) {
        return String.valueOf(new char[]{SETS[0].charAt(getInt(bytes, new byte[]{(byte) 39,
                (byte) 40, (byte) 41, (byte) 42, (byte) 31, (byte) 32})), SETS[0].charAt(getInt
                (bytes, new byte[]{(byte) 33, (byte) 34, (byte) 35, (byte) 36, (byte) 25,
                        JSONLexer.EOI})), SETS[0].charAt(getInt(bytes, new byte[]{(byte) 27,
                (byte) 28, (byte) 29, (byte) 30, (byte) 19, (byte) 20})), SETS[0].charAt(getInt
                (bytes, new byte[]{(byte) 21, (byte) 22, (byte) 23, (byte) 24, (byte) 13, df.l}))
                , SETS[0].charAt(getInt(bytes, new byte[]{df.m, df.n, (byte) 17, (byte) 18,
                (byte) 7, (byte) 8})), SETS[0].charAt(getInt(bytes, new byte[]{(byte) 9, (byte)
                10, (byte) 11, (byte) 12, (byte) 1, (byte) 2}))});
    }
        ...
        while (i < start + len) {
            ...
            switch (c) {
                ...
                case '￻':
                    ...
                    i++;
                    i++;
                    sb.append(NINE_DIGITS.format((long) (((((bytes[i] << 24) + (bytes[i] << 18))
                            + (bytes[i] << 12)) + (bytes[i] << 6)) + bytes[i])));
                    shift2 = shift;
                    break;
                default:
                    sb.append(c);
