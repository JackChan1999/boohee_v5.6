...
final class BitMatrixParser {
    ...
    byte[] readCodewords() throws FormatException {
        ...
        while (true) {
            ...
            if (row == numRows && column == 0 && !corner1Read) {
                ...
                resultOffset = resultOffset2;
                while (true) {
                    if (row < 0 || column >= numColumns || this.readMappingMatrix.get(column,
                            row)) {
                        resultOffset2 = resultOffset;
                    } else {
                        resultOffset2 = resultOffset + 1;
    ...
    BitMatrix extractDataRegion(BitMatrix bitMatrix) {
        ...
        BitMatrix bitMatrixWithoutAlignment = new BitMatrix(numDataRegionsColumn *
                dataRegionSizeColumns, numDataRegionsRow * dataRegionSizeRows);
        for (int dataRegionRow = 0; dataRegionRow < numDataRegionsRow; dataRegionRow++) {
            int dataRegionRowOffset = dataRegionRow * dataRegionSizeRows;
            for (int dataRegionColumn = 0; dataRegionColumn < numDataRegionsColumn;
                 dataRegionColumn++) {
                ...
                for (int i = 0; i < dataRegionSizeRows; i++) {
                    ...
                    for (int j = 0; j < dataRegionSizeColumns; j++) {
                        if (bitMatrix.get((((dataRegionSizeColumns + 2) * dataRegionColumn) + 1)
                                + j, readRowOffset)) {
                            bitMatrixWithoutAlignment.set(dataRegionColumnOffset + j,
                                    writeRowOffset);
                        }
                    }
                }
