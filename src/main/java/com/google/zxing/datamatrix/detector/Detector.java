...
public final class Detector {
    ...
    private static final class ResultPointsAndTransitionsComparator implements
    ...
    public DetectorResult detect() throws NotFoundException {
        ...
        if (dimensionTop * 4 >= dimensionRight * 7 || dimensionRight * 4 >= dimensionTop * 7) {
            ...
            bits = sampleGrid(this.image, topLeft, bottomLeft, bottomRight, correctedTopRight,
                    dimensionTop, dimensionRight);
        } else {
            correctedTopRight = correctTopRight(bottomLeft, bottomRight, topLeft, topRight, Math
                    .min(dimensionRight, dimensionTop));
            if (correctedTopRight == null) {
                correctedTopRight = topRight;
            }
        ...
        return new DetectorResult(bits, new ResultPoint[]{topLeft, bottomLeft, bottomRight,
                correctedTopRight});
    }
    ...
    private boolean isValid(ResultPoint p) {
        return p.getX() >= 0.0f && p.getX() < ((float) this.image.getWidth()) && p.getY() > 0.0f
                && p.getY() < ((float) this.image.getHeight());
    }
