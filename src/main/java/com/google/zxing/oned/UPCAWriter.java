...
public final class UPCAWriter implements Writer {
    ...
    public BitMatrix encode(String contents, BarcodeFormat format, int width, int height,
        ...
        if (format == BarcodeFormat.UPC_A) {
            return this.subWriter.encode(preencode(contents), BarcodeFormat.EAN_13, width,
                    height, hints);
        }
    ...
    private static String preencode(String contents) {
        ...
        if (length == 11) {
            ...
            throw new IllegalArgumentException("Requested contents should be 11 or 12 digits " +
                    "long, but got " + contents.length());
        }
        return '0' + contents;
    }
}
