...
final class GeneralAppIdDecoder {
        ...
        while (isStillNumeric(this.current.getPosition())) {
            ...
            if (numeric.isFirstDigitFNC1()) {
                ...
                if (numeric.isSecondDigitFNC1()) {
                    information = new DecodedInformation(this.current.getPosition(), this.buffer
                            .toString());
                } else {
                    information = new DecodedInformation(this.current.getPosition(), this.buffer
                            .toString(), numeric.getSecondDigit());
                }
                return new BlockParsedResult(information, true);
            }
            this.buffer.append(numeric.getFirstDigit());
            if (numeric.isSecondDigitFNC1()) {
                return new BlockParsedResult(new DecodedInformation(this.current.getPosition(),
                        this.buffer.toString()), true);
            }
    ...
    private boolean isStillIsoIec646(int pos) {
        ...
        if (sevenBitValue >= 64 && sevenBitValue < Opcodes.INVOKE_VIRTUAL_RANGE) {
            return true;
        }
        ...
        if (eightBitValue < 232 || eightBitValue >= SampleTinkerReport
    ...
    private DecodedChar decodeIsoIec646(int pos) throws FormatException {
        ...
        if (sevenBitValue >= 90 && sevenBitValue < Opcodes.INVOKE_VIRTUAL_RANGE) {
    ...
    private DecodedChar decodeAlphanumeric(int pos) {
        ...
        switch (sixBitValue) {
            ...
            case 62:
                ...
                break;
            default:
                throw new IllegalStateException("Decoding invalid alphanumeric value: " +
                        sixBitValue);
        }
