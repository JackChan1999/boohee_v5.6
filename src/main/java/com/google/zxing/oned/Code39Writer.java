...
public final class Code39Writer extends OneDimensionalCodeWriter {
    ...
    public boolean[] encode(String contents) {
        ...
        if (length > 80) {
            throw new IllegalArgumentException("Requested contents should be less than 80 digits " +
                    "long, but got " + length);
        }
        int i;
        ...
        for (i = 0; i < length; i++) {
            int indexInString = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ-. *$/+%".indexOf(contents
                    .charAt(i));
            if (indexInString < 0) {
        ...
        for (i = 0; i < length; i++) {
            toIntArray(Code39Reader.CHARACTER_ENCODINGS[("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ-. " +
                    "*$/+%").indexOf(contents.charAt(i))], widths);
