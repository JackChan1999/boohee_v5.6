...
final class AlignmentPatternFinder {
    ...
    AlignmentPatternFinder(BitMatrix image, int startX, int startY, int width, int height, float
            moduleSize, ResultPointCallback resultPointCallback) {
        this.image = image;
        this.startX = startX;
        this.startY = startY;
        this.width = width;
    ...
    private AlignmentPattern handlePossibleCenter(int[] stateCount, int i, int j) {
        ...
        if (!Float.isNaN(centerI)) {
            float estimatedModuleSize = ((float) ((stateCount[0] + stateCount[1]) +
                    stateCount[2])) / IPhotoView.DEFAULT_MAX_SCALE;
