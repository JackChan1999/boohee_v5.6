...
final class DataBlock {
    ...
    static DataBlock[] getDataBlocks(byte[] rawCodewords, Version version, ErrorCorrectionLevel
            ecLevel) {
        ...
        for (ECB ecBlock2 : ecBlockArray) {
            ...
            while (i < ecBlock2.getCount()) {
                ...
                result[numResultBlocks] = new DataBlock(numDataCodewords, new byte[(ecBlocks
                        .getECCodewordsPerBlock() + numDataCodewords)]);
                i++;
        ...
        while (longerBlocksStartAt >= 0 && result[longerBlocksStartAt].codewords.length !=
                shorterBlocksTotalCodewords) {
            longerBlocksStartAt--;
        }
        longerBlocksStartAt++;
        int shorterBlocksNumDataCodewords = shorterBlocksTotalCodewords - ecBlocks
        ...
        while (i < max) {
            ...
            while (j < numResultBlocks) {
                rawCodewordsOffset2 = rawCodewordsOffset + 1;
                result[j].codewords[j < longerBlocksStartAt ? i : i + 1] =
