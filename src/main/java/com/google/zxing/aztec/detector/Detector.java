...
public final class Detector {
    ...
    private void extractParameters(ResultPoint[] bullsEyeCorners) throws NotFoundException {
        if (isValid(bullsEyeCorners[0]) && isValid(bullsEyeCorners[1]) && isValid
                (bullsEyeCorners[2]) && isValid(bullsEyeCorners[3])) {
            int[] sides = new int[]{sampleLine(bullsEyeCorners[0], bullsEyeCorners[1], length),
                    sampleLine(bullsEyeCorners[1], bullsEyeCorners[2], length), sampleLine
                    (bullsEyeCorners[2], bullsEyeCorners[3], length), sampleLine
                    (bullsEyeCorners[3], bullsEyeCorners[0], this.nbCenterLayers * 2)};
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (this.compact) {
                    ...
                    parameterData = (parameterData << 10) + ((long) (((side >> 2) & 992) + ((side
                            >> 1) & 31)));
                }
            }
    ...
    private static int getCorrectedParameterData(long parameterData, boolean compact) throws
            NotFoundException {
        int numCodewords;
    ...
    private ResultPoint[] getBullsEyeCorners(Point pCenter) throws NotFoundException {
        ...
        while (this.nbCenterLayers < 9) {
            ...
            if (this.nbCenterLayers > 2) {
                float q = (distance(poutd, pouta) * ((float) this.nbCenterLayers)) / (distance
                        (pind, pina) * ((float) (this.nbCenterLayers + 2)));
                if (((double) q) >= 0.75d) {
                    if (((double) q) <= 1.25d) {
        ...
        if (this.nbCenterLayers == 5 || this.nbCenterLayers == 7) {
            ...
            ResultPoint pinax = new ResultPoint(((float) pina.getX()) + 0.5f, ((float) pina.getY
                    ()) - 0.5f);
            ResultPoint pinbx = new ResultPoint(((float) pinb.getX()) + 0.5f, ((float) pinb.getY
                    ()) + 0.5f);
            ResultPoint pincx = new ResultPoint(((float) pinc.getX()) - 0.5f, ((float) pinc.getY
                    ()) + 0.5f);
            ResultPoint pindx = new ResultPoint(((float) pind.getX()) - 0.5f, ((float) pind.getY
                    ()) - 0.5f);
            return expandSquare(new ResultPoint[]{pinax, pinbx, pincx, pindx}, (float) ((this
                    .nbCenterLayers * 2) - 3), (float) (this.nbCenterLayers * 2));
        }
        throw NotFoundException.getNotFoundInstance();
    }
    ...
    private ResultPoint[] getMatrixCornerPoints(ResultPoint[] bullsEyeCorners) {
        return expandSquare(bullsEyeCorners, (float) (this.nbCenterLayers * 2), (float)
                getDimension());
    }
    ...
    private boolean isWhiteOrBlackRectangle(Point p1, Point p2, Point p3, Point p4) {
        ...
        if (cInit != 0 && getColor(p12, p22) == cInit && getColor(p22, p32) == cInit && getColor
                (p32, p42) == cInit) {
            return true;
        }
        return false;
    ...
    private static ResultPoint[] expandSquare(ResultPoint[] cornerPoints, float oldSide, float
            newSide) {
