...
public final class Encoder {
    ...
    private static final int[] WORD_SIZE            = new int[]{4, 6, 6, 8, 8, 8, 8, 8, 8, 10,
            10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 12, 12, 12, 12, 12, 12, 12, 12,
            12, 12};
    ...
    public static AztecCode encode(byte[] data, int minECCPercent, int userSpecifiedLayers) {
        ...
        if (userSpecifiedLayers != 0) {
            ...
            if (layers > (compact ? 4 : 32)) {
                throw new IllegalArgumentException(String.format("Illegal value %s for layers",
                        new Object[]{Integer.valueOf(userSpecifiedLayers)}));
            }
        ...
        while (i <= 32) {
            ...
            if (totalSizeBits <= totalBitsInLayer) {
                ...
                if ((!compact || stuffedBits.getSize() <= wordSize * 64) && stuffedBits.getSize()
                        + eccBits <= usableBitsInLayers) {
                }
            }
            i++;
        }
            ...
            for (j = 0; j < rowSize; j++) {
                ...
                for (k = 0; k < 2; k++) {
                    ...
                    if (messageBits.get((((rowSize * 2) + rowOffset) + columnOffset) + k)) {
                        matrix.set(alignmentMap[(i * 2) + j], alignmentMap[((baseMatrixSize - 1)
                                - (i * 2)) - k]);
                    }
