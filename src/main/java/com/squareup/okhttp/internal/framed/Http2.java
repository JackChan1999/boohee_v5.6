...
public final class Http2 implements Variant {
    private static final ByteString CONNECTION_PREFACE     = ByteString.encodeUtf8("PRI * HTTP/2" +
            ".0\r\n\r\nSM\r\n\r\n");
    ...
    static final class ContinuationSource implements Source {
        ...
        private void readContinuationHeader() throws IOException {
            ...
            if (Http2.logger.isLoggable(Level.FINE)) {
                Http2.logger.fine(FrameLogger.formatHeader(true, this.streamId, this.length,
                        type, this.flags));
            }
            this.streamId = this.source.readInt() & ActivityChooserViewAdapter
    ...
    static final class FrameLogger {
        ...
        private static final String[] TYPES  = new String[]{"DATA", "HEADERS", "PRIORITY",
        ...
        static String formatHeader(boolean inbound, int streamId, int length, byte type, byte
                flags) {
            String formattedType = type < TYPES.length ? TYPES[type] : String.format("0x%02x",
            ...
            switch (type) {
                ...
                default:
                    ...
                    if (type != (byte) 5 || (flags & 4) == 0) {
                        return (type != (byte) 0 || (flags & 32) == 0) ? result : result.replace
                                ("PRIORITY", "COMPRESSED");
                    } else {
        ...
        static {
            ...
            for (i = 0; i < BINARY.length; i++) {
                BINARY[i] = String.format("%8s", new Object[]{Integer.toBinaryString(i)}).replace
                        (' ', '0');
            }
            FLAGS[0] = "";
            FLAGS[1] = "END_STREAM";
            ...
            for (int frameFlag : new int[]{4, 32, 36}) {
                for (int prefixFlag2 : prefixFlags) {
                    ...
                    FLAGS[(prefixFlag2 | frameFlag) | 8] = FLAGS[prefixFlag2] + '|' +
    ...
    static final class Reader implements FrameReader {
        ...
        public void readConnectionPreface() throws IOException {
            if (!this.client) {
                ByteString connectionPreface = this.source.readByteString((long) Http2
                        .CONNECTION_PREFACE.size());
                if (Http2.logger.isLoggable(Level.FINE)) {
                    Http2.logger.fine(String.format("<< CONNECTION %s", new
                            Object[]{connectionPreface.hex()}));
                }
        ...
        public boolean nextFrame(Handler handler) throws IOException {
            try {
                ...
                int streamId = this.source.readInt() & ActivityChooserViewAdapter
                        .MAX_ACTIVITY_COUNT_UNLIMITED;
                if (Http2.logger.isLoggable(Level.FINE)) {
                    Http2.logger.fine(FrameLogger.formatHeader(true, streamId, length, type,
                            flags));
                }
                switch (type) {
                    case (byte) 0:
        ...
        private void readHeaders(Handler handler, int length, byte flags, int streamId) throws
                IOException {
            if (streamId == 0) {
                throw Http2.ioException("PROTOCOL_ERROR: TYPE_HEADERS streamId == 0", new
                        Object[0]);
            }
            boolean endStream;
            short padding;
            ...
            handler.headers(false, endStream, streamId, -1, readHeaderBlock(Http2
                    .lengthWithoutPadding(length, flags, padding), padding, flags, streamId),
        ...
        private void readData(Handler handler, int length, byte flags, int streamId) throws
                IOException {
            boolean inFinished;
            ...
            if (gzipped) {
                throw Http2.ioException("PROTOCOL_ERROR: FLAG_COMPRESSED without " +
                        "SETTINGS_COMPRESS_DATA", new Object[0]);
            }
            if ((flags & 8) != 0) {
                padding = (short) (this.source.readByte() & 255);
            }
            handler.data(inFinished, streamId, this.source, Http2.lengthWithoutPadding(length,
                    flags, padding));
            this.source.skip((long) padding);
        }

        private void readPriority(Handler handler, int length, byte flags, int streamId) throws
                IOException {
            if (length != 5) {
        ...
        private void readPriority(Handler handler, int streamId) throws IOException {
            ...
            handler.priority(streamId, w1 & ActivityChooserViewAdapter
                    .MAX_ACTIVITY_COUNT_UNLIMITED, (this.source.readByte() & 255) + 1, (Integer
                    .MIN_VALUE & w1) != 0);
        }

        private void readRstStream(Handler handler, int length, byte flags, int streamId) throws
                IOException {
            if (length != 4) {
                ...
                if (errorCode == null) {
                    throw Http2.ioException("TYPE_RST_STREAM unexpected error code: %d", Integer
                            .valueOf(errorCodeInt));
                } else {
        ...
        private void readPushPromise(Handler handler, int length, byte flags, int streamId)
            ...
            short padding = (short) 0;
            if (streamId == 0) {
                throw Http2.ioException("PROTOCOL_ERROR: TYPE_PUSH_PROMISE streamId == 0", new
                        Object[0]);
            }
            if ((flags & 8) != 0) {
                padding = (short) (this.source.readByte() & 255);
            }
            handler.pushPromise(streamId, this.source.readInt() & ActivityChooserViewAdapter
                    .MAX_ACTIVITY_COUNT_UNLIMITED, readHeaderBlock(Http2.lengthWithoutPadding
                    (length - 4, flags, padding), padding, flags, streamId));
        }

        private void readPing(Handler handler, int length, byte flags, int streamId) throws
                IOException {
            boolean ack = true;
            if (length != 8) {
        ...
        private void readGoAway(Handler handler, int length, byte flags, int streamId) throws
                IOException {
            if (length < 8) {
                ...
                ErrorCode errorCode = ErrorCode.fromHttp2(this.source.readInt());
                if (errorCode == null) {
                    throw Http2.ioException("TYPE_GOAWAY unexpected error code: %d", Integer
                            .valueOf(errorCodeInt));
                }
        ...
        private void readWindowUpdate(Handler handler, int length, byte flags, int streamId)
                throws IOException {
            if (length != 4) {
                throw Http2.ioException("TYPE_WINDOW_UPDATE length !=4: %s", Integer.valueOf
                        (length));
            }
    ...
    static final class Writer implements FrameWriter {
        ...
        public synchronized void connectionPreface() throws IOException {
            if (this.closed) {
                ...
                if (Http2.logger.isLoggable(Level.FINE)) {
                    Http2.logger.fine(String.format(">> CONNECTION %s", new Object[]{Http2
                            .CONNECTION_PREFACE.hex()}));
                }
        ...
        public synchronized void pushPromise(int streamId, int promisedStreamId, List<Header>
            ...
            frameHeader(streamId, length + 4, (byte) 5, byteCount == ((long) length) ? (byte) 4 :
                    (byte) 0);
        ...
        void headers(boolean outFinished, int streamId, List<Header> headerBlock) throws
                IOException {
            if (this.closed) {
                throw new IOException("closed");
            }
        ...
        public synchronized void windowUpdate(int streamId, long windowSizeIncrement) throws
            ...
            if (this.closed) {
                ...
                throw Http2.illegalArgument("windowSizeIncrement == 0 || windowSizeIncrement > " +
                        "0x7fffffffL: %s", Long.valueOf(windowSizeIncrement));
            } else {
        ...
        void frameHeader(int streamId, int length, byte type, byte flags) throws IOException {
            ...
            if (length > this.maxFrameSize) {
                throw Http2.illegalArgument("FRAME_SIZE_ERROR length > %d: %d", Integer.valueOf
                        (this.maxFrameSize), Integer.valueOf(length));
    ...
    private static int lengthWithoutPadding(int length, byte flags, short padding) throws
            IOException {
        if ((flags & 8) != 0) {
